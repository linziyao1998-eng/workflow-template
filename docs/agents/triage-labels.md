...
| `needs-info`        | `D93F0B` | Material requirement gaps remain, or a new external-source intake awaits full-spec confirmation. |
...
For a new Issue created from a conversation, material or another repository, record the human clarification exchange before creation and apply `needs-info` as its initial primary state. Keep it through source clarification and full-spec confirmation, with the pending reason recorded in the Issue. When new information arrives, reassess every applicable readiness item in that same invocation. A complete, approved agent-suitable version may move directly from `needs-info` to `ready-for-agent` with `Spec` after exact-version readback.

For an Issue that already existed in this tracker, an answered gap moves to `needs-triage` for reassessment. Remaining material gaps return it to `needs-info`; incomplete evaluation or missing full approval leaves it at `needs-triage`; fully approved agent-suitable work may receive `ready-for-agent`. Do not infer approval from a label, silence or an agent's confidence.
