...
For an external source without a target Issue, complete the clarification phase and incorporate all currently confirmable information before applying the minimum intake gate in `workflow-clarify-spec/references/carriers.md`. Below that minimum, do not create an Issue. If material gaps remain after clarification but the minimum is met, create the Spec Issue with `needs-info`. If the requirements are complete, obtain separate full-spec approval before creating the Issue; its verified final state is `Spec` + `ready-for-agent`. A workflow-created Issue that began with gaps loses `needs-info` once its updated body has no material gaps, then awaits full approval without a primary triage label. This route never uses `needs-triage`.
