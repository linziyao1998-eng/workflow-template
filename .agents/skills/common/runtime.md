...
For an external source with no target Issue, first complete the clarification phase and incorporate every fact currently confirmable from the source and human answers. Then apply the minimum intake gate before any Issue creation. Below minimum → no Issue; minimum with remaining material gaps → Issue with `needs-info`; complete requirements → separate full-spec approval before creating an Issue that is then verified and labeled `Spec` + `ready-for-agent`. A workflow-created Issue opened with gaps keeps `needs-info` only while those gaps remain; after they are resolved, update its body, remove `needs-info`, and obtain full approval before applying readiness. Never apply `needs-triage` on this route. Reassess all applicable readiness items whenever an answer arrives in the same invocation. For an Issue that already existed in the target tracker, an answered gap moves to needs-triage for full reassessment: remaining material gaps → needs-info; incomplete evaluation or missing full approval → needs-triage. Complete and approved work requiring human implementation → ready-for-human. API contract completeness and dev-service availability are separate; a permitted fixture strategy may make the latter nonblocking.
