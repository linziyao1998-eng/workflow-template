...
For a workflow-created Issue sourced outside the target tracker, apply `needs-info` only while material requirement gaps remain. Once those gaps are resolved, remove `needs-info` and leave the Issue without a primary triage label until full-spec approval; never apply `needs-triage` on this path. Reassess all applicable readiness items whenever an answer arrives in the same invocation. For an Issue that already existed in the target tracker, an answered gap moves to needs-triage for full reassessment: remaining material gaps → needs-info; incomplete evaluation or missing full approval → needs-triage. Complete and approved work requiring human implementation → ready-for-human. Only complete, approved, agent-suitable specs can receive Spec + ready-for-agent. API contract completeness and dev-service availability are separate; a permitted fixture strategy may make the latter nonblocking.
