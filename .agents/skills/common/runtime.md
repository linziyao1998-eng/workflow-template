...
For a workflow-created Issue sourced outside the target tracker, apply `needs-info` at creation and keep it while source questions or full-spec confirmation remain. Reassess all applicable readiness items whenever an answer arrives in the same invocation; do not use a temporary `needs-triage` transition on this intake path. For an Issue that already existed in the target tracker, an answered gap moves to needs-triage for full reassessment: remaining material gaps → needs-info; incomplete evaluation or missing full approval → needs-triage. Complete work requiring human implementation → ready-for-human. Only complete, approved, agent-suitable specs can receive Spec + ready-for-agent. API contract completeness and dev-service availability are separate; a permitted fixture strategy may make the latter nonblocking.
