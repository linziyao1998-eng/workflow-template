...
- **Conversation, materials or another repo's Issue, with no target tracker Issue:** treat these as external sources for a workflow-created target Issue. First show the source-bound understanding, conduct at least one substantive clarification round and wait for the human's answer. Invoking the skill, supplying credentials or accepting an agent-generated document is not that round. Apply the minimum intake gate in [carriers.md](references/carriers.md):
  1. Below the minimum: report the precise missing facts and create no target Issue.
  2. Minimum met, material requirements still missing: create or resume an incomplete seven-section Issue with `needs-info`, recording each gap and question.
